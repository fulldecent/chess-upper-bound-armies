...
//TODO FD: count non castling, king anywhere, then castling, with one, then two rooks fixed ??
...
//FD Revelation: all files are originally blocked, promotion potential comes from
//  pawns leaving this blocked file
...
main()
{
        ...
        for (nq=0; nq<=MAX_QUEENS; nq++)
        {
            for (nr=0; nr<=MAX_ROOKS; nr++)
            {
                for (nb=0; nb<=MAX_BISHOPS; nb++)
                {
                    ...
                    {
                        pieces[ncombos].promos = max(nq-1,0) + max(nr-2,0) + max(nb-2,0) + max(nn-2,0);
                        if (pieces[ncombos].promos > 8 - np)
                            continue;

                        pieces[ncombos].n = np + nq + nr + nb + nn + 1;  /* K inc'd */
                        pieces[ncombos].np = np;
                        pieces[ncombos].nq = nq;
                        pieces[ncombos].nr = nr;
                        pieces[ncombos].nb = nb;
                        pieces[ncombos].nn = nn;
                        pieces[ncombos].prod = fs[nq] * fs[nr] * fs[nb] * fs[nn];
                        ncombos++;
    ...
    for (i=0; i<ncombos; i++)
    {
        ...
        nWpawns_captured = 8 - pieces[i].np - pieces[i].promos;
        nWpieces_captured = nWcaptures - nWpawns_captured;

        if (pieces[i].promos < 0 || nWcaptures < 0 || nWpawns_captured < 0 ||
        ...
        for (j=0; j<ncombos; j++)
        {
            ...
            npromos = pieces[i].promos + pieces[j].promos;
            nBcaptures = 16 - pieces[j].n;
            nBpawns_captured = 8 - pieces[j].np - pieces[j].promos;
            nBpieces_captured = nBcaptures - nBpawns_captured;

            if (pieces[j].promos < 0 || nBcaptures < 0 || nBpawns_captured < 0 ||
